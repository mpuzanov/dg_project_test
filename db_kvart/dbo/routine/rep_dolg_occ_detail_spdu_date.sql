...
-- Description:	
-- =============================================
CREATE         PROCEDURE [dbo].[rep_dolg_occ_detail_spdu_date]
	@occ		INT
	,@fin_id1	SMALLINT
	,@date1		SMALLDATETIME
	,@date2		SMALLDATETIME
...
exec [rep_dolg_occ_detail_spdu_date] 700014333,182,'20170301','20170312',0
...
BEGIN
	...
	DELETE FROM #paym_list1 WHERE value=0 AND kol=0 AND added=0
	...
	SELECT 
	*
	,[Начислено_Итог]=SUM([Начислено]) OVER ()
	,[Итого начисл._Итог]=SUM([Итого начисл.]) OVER ()
	FROM (		
	SELECT
		serv_name AS 'Услуга'
		,sort_no
		...
		,CASE
				...
				ELSE ROUND(SUM(kol*@koefDay), 4)
		END AS 'Кол-во'
		,SUM(saldo) AS [Вх.Сальдо]
		--,SUM(value*@koefDay) AS 'Начислено'
		--,SUM(CONVERT(DECIMAL(9,2),value*@koefDay)) AS 'Начислено'
		,ROUND(SUM(value*@koefDay),2) AS [Начислено]
		,SUM(added) AS [Перерасчет]
		,ROUND(SUM((value*@koefDay)+added),2) AS [Итого начисл.]
		,SUM(paymaccount) AS [Оплатил]
		,SUM(paymaccount_peny) AS [из них пени]
		,SUM(saldo+(ROUND((value*@koefDay),2)+added)-(paymaccount-paymaccount_peny)) AS [Кон. сальдо]
	FROM #paym_list1 p
