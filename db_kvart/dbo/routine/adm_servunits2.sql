CREATE   PROCEDURE [dbo].[adm_servunits2]
(
...
AS
	/*
	adm_servunits2 212, 28, 'площ', 'отдк'
	adm_servunits2 212, 28, 'площ'
	*/
	SET NOCOUNT ON
	...
	SELECT
	   ...
	   ,service_id
	   ,MAX(CASE
			WHEN roomtype_id = 'отдк' THEN unit_id
			ELSE NULL
		END) AS unit_id_otd
	   ,MAX(CASE
			WHEN roomtype_id = 'комм' THEN unit_id
			ELSE NULL
		END) AS unit_id_kom
	   ,MAX(CASE
			WHEN roomtype_id = 'об10' THEN unit_id
			ELSE NULL
		END) AS unit_id_ob10
	   ,MAX(CASE
			WHEN roomtype_id = 'об06' THEN unit_id
			ELSE NULL
		END) AS unit_id_ob6
