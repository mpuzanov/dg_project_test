...
-- =============================================
CREATE         PROCEDURE [dbo].[k_peny_dolg_occ_2016]
(
	  @occ INT
	, @sup_id INT = 0
...
exec k_peny_dolg_occ_2016 @occ=700009304,@sup_id=0,@debug=1
...
BEGIN
	...
	DECLARE @mes DECIMAL(5, 1) = 0
		  , @dolg_ost DECIMAL(15, 2)
		  , @paid DECIMAL(15, 2)
		  , @saldo DECIMAL(15, 2)
		  , @sum_pay DECIMAL(15, 2)
		  , @sum_pay_current DECIMAL(15, 2)
		  , @paymaccount DECIMAL(15, 2)
		  , @paymaccount_tmp DECIMAL(15, 2)
		  , @fin_id SMALLINT
		  , @fin_pred SMALLINT
	...
	IF @debug = 1
		PRINT 'Долг: ' + STR(@dolg, 9, 2) + ', Оплата: ' + STR(@sum_pay, 9, 2) + ', Тек.оплата: ' + STR(@sum_pay_current, 9, 2)
	IF @debug = 1
		SELECT '@t' AS '@t'
			 , *
		FROM @t
	...
	SELECT @dolg_ost = @dolg - @sum_pay_current
	...
	BEGIN
		...
		UPDATE @t
		SET dolg = @paid - @paymaccount
		WHERE fin_id = @fin_id

		IF @debug = 1
			PRINT STR(@fin_id) + ' ' + STR(@dolg_ost, 9, 2) + ' ' + STR(@paid, 9, 2) + ' ' + STR(@paymaccount, 9, 2)
		...
		SET @dolg_ost = @dolg_ost - @paid - @paymaccount
	...
	SELECT @dolg_ost = SUM(dolg)
	FROM @t

	IF @dolg_ost >= 0
	BEGIN
		IF @debug = 1
			PRINT '@dolg_ost >= 0   @dolg_ost=' + STR(@dolg_ost, 9, 2) + ' @dolg=' + STR(@dolg, 9, 2)
		UPDATE t
