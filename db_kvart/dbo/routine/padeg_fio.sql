...
-- =============================================
CREATE         PROCEDURE [dbo].[padeg_fio]
(
...
AS
/*
кто, что?	Именительный:		Иванов Иван Иванович
кому, чему?	Дательный ('Д'):	Иванову Ивану Ивановичу		- справка выдана кому?
кого, чего?	Родительный ('Р'):	Иванова Ивана Ивановича		- заявление от кого?
кем, чем?	Творительный ('Т'):	Ивановым Иваном Ивановичем  - проживает совместно с кем?

EXEC padeg_fio @Last_name='ИВАНОВ', @First_name='ИВАН', @Second_name='ИВАНОВИЧ', @padeg='Д', @sex='МУЖ', @debug=1
EXEC padeg_fio @Last_name='ИВАНОВ', @First_name='ИВАН', @Second_name='ИВАНОВИЧ', @padeg='Р', @sex='МУЖ', @debug=1
EXEC padeg_fio @Last_name='ИВАНОВА', @First_name='НАТАЛЬЯ', @Second_name='ВЛАДИМИРОВНА', @padeg='Т', @sex='ЖЕН', @debug=1

В русском алфавите всего 33 буквы, 
из них 10 – это гласные (а, е, ё, и, о, у, ы, э, ю, я), 
а 21 – это согласные (б, в, г, д, ж, з, й, к, л, м, н, п, р, с, т, ф, х, ц, ч, ш, щ), 
а также два знака: ъ и ь.

прочитать особенности для разработки
http://kozin1.narod.ru/sql/fio.html

https://surnameonline.ru/
*/
BEGIN
	SET NOCOUNT ON;
	...
	SELECT @Last_name = UPPER(LEFT(@Last_name, 1))+SUBSTRING(@Last_name,2,30)
	...
	SELECT @padeg=COALESCE(@padeg,'Д'), @sex = COALESCE(UPPER(@sex),'МУЖ'), @debug = COALESCE(@debug,0)
	...
	DECLARE @except TABLE (
		...
		val NVARCHAR(100),  -- в именительном пдеже
		val_dat NVARCHAR(100), -- в дательном
		val_rod NVARCHAR(100), -- в родительном
		val_tvor NVARCHAR(100), -- в творительном
