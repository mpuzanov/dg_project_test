CREATE   PROCEDURE [dbo].[k_intPrintDetail_occ_build]
(
	...
	, @Sup_Id INT = 0
	, @Debug BIT = 0
	, @is_out_gis BIT = NULL -- выгрузка в гис 
)
/*
Выдаем информацию по услугам для единой квитанции
с общедомовыми услугами

exec k_intPrintDetail_occ_build @Fin_Id1=231,@build_id=6785,@Occ1=31001
exec k_intPrintDetail_occ_build 231,6785,31001,1,345,0 
exec k_intPrintDetail_occ_build 231,6785,NULL,1,0,0 
exec k_intPrintDetail_occ_build 176,NULL,680002965,28,347,1 
exec k_intPrintDetail_occ_build 139,NULL,680001163,28,323,1
...
AS
	...
	SELECT @is_out_gis=COALESCE(@is_out_gis, 0)
	...
	BEGIN TRY
		...
		CREATE TABLE #t(
			...
			, reason_added VARCHAR(800) COLLATE database_default DEFAULT NULL -- причина перерасчёта по услугам
		...
		--CREATE NONCLUSTERED INDEX occ_serv ON #t (occ,service_id) INCLUDE (sup_id, unit_id)
		...
		BEGIN
			...
			SELECT f.bldn_id
                ...
                END AS short_id
				 ...
				 , COALESCE(ROUND(p.kol, COALESCE(u.precision, 4)), 0) AS kol
				 , p.koef
				 , p.saldo
				 , p.value
				 , p.added
				 ...
				 , st.blocked_kvit
				 , p.penalty_serv
				 , SUM(p.value) OVER(PARTITION BY p.occ) AS value_occ
		...
		BEGIN
			...
			FROM dbo.Occ_history AS o 
				JOIN dbo.Flats f ON 
					o.flat_id = f.id
				LEFT JOIN dbo.Paym_history AS p ON 
					o.occ = p.occ
					AND o.fin_id = p.fin_id
				--AND s.id = p.service_id --AND (p.subsid_only = 0)
				JOIN dbo.Services AS s ON 
					s.id = p.service_id --AND (p.subsid_only = 0)
			...
			FROM #t AS t
				JOIN dbo.Cons_modes_history AS cm ON 
					t.mode_id = cm.mode_id
				JOIN dbo.Units AS u ON 
					cm.unit_id = u.id
		...
		FROM #t AS t
			JOIN dbo.Services_types AS sb ON 
				t.OWNER_ID = sb.id
		...
		FROM #t AS t
			JOIN dbo.Services_build AS sb ON 
				t.service_id = sb.service_id
		...
		SET unit_id = u.id
		  , short_id = u.short_id
		FROM #t AS t
			JOIN dbo.Service_units su ON 
				su.service_id = t.service_id
			JOIN dbo.Units u ON 
				u.id = su.unit_id
		...
		if @is_out_gis = 1
		BEGIN
			DECLARE @is_export_gis_without_paid BIT = 0 -- экспорт услуг без начислений
			SELECT @is_export_gis_without_paid=is_export_gis_without_paid FROM dbo.OCCUPATION_TYPES WHERE id = @Tip_Id

			IF @is_export_gis_without_paid=0
				DELETE FROM #t   -- удаляем услуги где нет начислений
				--OUTPUT 'DELETE', DELETED.*
				WHERE paid_dom = 0
				AND value = 0
				AND added = 0
				AND paid = 0
			ELSE  -- оставляем услуги, где есть начисления или сальдо сальдо, остальные удаляем
			BEGIN
				DELETE FROM #t
				--OUTPUT 'DELETE', DELETED.*
				WHERE (value_occ=0 AND paid_dom = 0 AND value = 0 AND added = 0 AND paid = 0 AND saldo = 0 AND debt = 0)
				OR (value_occ<>0 AND paid_dom = 0 AND value = 0	AND added = 0 AND paid = 0)
		...
		ELSE
			...
			WHERE (paid_dom = 0
				...
				AND (VYDEL = 0 OR (VYDEL = 1 AND source_id % 1000 = 0)) -- если услугу надо показавать с нулевыми начислениями
				)  
		--================================
		...
		FROM #t AS t
			JOIN dbo.Services_types AS st ON 
				t.service_id = st.service_id
		...
		FROM #t AS t
			JOIN dbo.Services_types AS st ON 
				t.service_id = st.service_id   -- сначала по коду услуги
		WHERE st.tip_id = t.tip_id
		...
		FROM #t AS t
			JOIN dbo.Services_types AS st ON 
				t.short_name = st.service_name  -- потом по наименованию (допустим где кода услуги нет(группа))
		WHERE st.tip_id = t.tip_id
		...
		FROM #t AS t
			JOIN dbo.Services_build AS sb ON 
				(t.service_id = sb.service_id OR t.short_name = sb.service_name)  -- 31.07.2019 добавил OR T.short_name=sb.service_name
		WHERE sb.build_id = t.build_id
		...
		--IF @debug=1 SELECT 'без начислений',* FROM @t where value=0 and coalesce(service_name_gis,'')=''
		...
		FROM #t AS t
			...
			JOIN dbo.Group_kvit AS g ON 
				st.group_kvit_id = g.id
		WHERE t.OWNER_ID = 0
		...
		FROM #t AS t
			JOIN #t AS t2 ON 
				t.short_name = t2.service_name
		WHERE t.group_name_kvit = ''

		
		IF dbo.strpos('KR1', @DB_NAME) > 0 AND @is_out_gis=0
		...
		SELECT dbo.Fun_GetNumUV(occ, @Fin_Id1, @Sup_Id) AS num_pd
			 , *
		FROM #t
		WHERE COALESCE(sup_id, 0) = @Sup_Id
		ORDER BY occ
