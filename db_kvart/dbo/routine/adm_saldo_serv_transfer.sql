...
-- =============================================
CREATE             PROCEDURE [dbo].[adm_saldo_serv_transfer]
(
	  @tip_id SMALLINT
...
BEGIN
	...
	--BEGIN
	--	RAISERROR ('Поставщики должны быть заданы!', 11, 1);
	--	RETURN 1;
	--END

	IF (@service_new Collate Cyrillic_General_CS_AS = @service_old)
	...
	IF @debug = 1
		SELECT @mode_id_NOT AS mode_id_NOT
	...
	BEGIN TRY
		...
		BEGIN
			...
			IF @debug = 1
				SELECT CONCAT('Суммы не сходяться!(',LTRIM(STR((@Saldo_old + @Saldo_old2), 9, 2)),'<>',LTRIM(STR(@Saldo_new, 9, 2)),')' )
		END
		...
		SELECT @comments1 = CONCAT('с услуги: <',@service_old,'>-><',@service_new,'>')
