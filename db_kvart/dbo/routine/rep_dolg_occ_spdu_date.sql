...
-- =============================================
CREATE     PROCEDURE [dbo].[rep_dolg_occ_spdu_date]
	@OCC			INT			= NULL
	,@date1			SMALLDATETIME
	,@date2			SMALLDATETIME
...
exec [rep_dolg_occ_spdu_date] 680002998,'20141001','20150215',null,null
...
BEGIN
	...
	SELECT
		@date1 = dbo.Fun_GetOnlyDate(@date1)
		,@date2 = dbo.Fun_GetOnlyDate(@date2)
		...
		(	occ
			...
			,vid_blag)
				SELECT
					O.occ
					,O.address
					,dbo.Fun_DolgMesCalAll_date(O.occ, @fin_id1, @fin_id2) --i.KolMesDolgAll  --,i.KolMesDolg
					,dbo.Fun_Initials_All(O.occ)
					,B.vid_blag
	...
	(
		...
		,[Оплата по услугам]	AS ([Оплатил] - [из них пени])  --[DECIMAL](9, 2)		NULL
		,[Кон. сальдо]			[DECIMAL](9, 2)		NULL
		,[К оплате]				[DECIMAL](9, 2)		NULL
		,[Год]					[INT]				NULL
		,[Вид платежа]			[VARCHAR](50)		COLLATE database_default NULL
		,[address]				[VARCHAR](50)		COLLATE database_default NULL
	...
	INSERT INTO #Table1
			SELECT
				...
				--,CAST(p.Paymaccount_serv + (COALESCE(os.PaymAccount, 0) - COALESCE(os.PaymAccount_peny, 0)) AS DECIMAL(9, 2)) AS 'Оплата по услугам'
	...
	SET	[Оплатил]		=
		COALESCE((SELECT
			...
			WHERE p.occ = t1.occ
			AND p.fin_id = t1.fin_id
			AND p.day BETWEEN @date1 AND @date2)
		, 0)
		,[из них пени]	=
		COALESCE((SELECT
				SUM(p.paymaccount_peny)
			FROM dbo.View_PAYINGS p 
			WHERE p.occ = t1.occ
			AND p.fin_id = t1.fin_id
			AND p.day BETWEEN @date1 AND @date2)
	...
	BEGIN
		...
		UPDATE t1
		SET [Начислено] = ROUND([Начислено] * t1.koefDay,2)
		FROM #Table1 AS t1
		WHERE fin_id = @var1
