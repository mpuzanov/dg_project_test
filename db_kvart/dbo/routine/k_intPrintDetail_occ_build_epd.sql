CREATE   PROCEDURE [dbo].[k_intPrintDetail_occ_build_epd]
(
...
AS
	...
	BEGIN TRY
		...
		CREATE TABLE #T(
			...
			, penalty_serv DECIMAL(15, 2) DEFAULT 0			
			, penalty_prev DECIMAL(15, 2) DEFAULT 0
			, paymaccount DECIMAL(15, 2) DEFAULT 0
			, paymaccount_peny DECIMAL(15, 2) DEFAULT 0
		)
		...
		BEGIN
			...
			SELECT f.bldn_id
                ...
                END AS short_id
				 ...
				 , p.koef
				 , p.saldo + p.penalty_prev
				 , p.value
				 , p.added
				 , p.paid
				 , p.debt + p.penalty_prev - p.paymaccount_peny
		...
		BEGIN
			...
			SELECT f.bldn_id
                ...
                END AS short_id
				 ...
				 , COALESCE(ROUND(p.kol, COALESCE(u.precision, 4)), 0) AS kol
				 , p.koef
				 , COALESCE(p.saldo, 0) + COALESCE(p.penalty_prev, 0)
				 , COALESCE(p.value, 0)
				 , COALESCE(p.added, 0)
				 , COALESCE(p.paid, 0)
				 , COALESCE(p.debt, 0) + COALESCE(p.penalty_prev, 0) - COALESCE(p.paymaccount_peny, 0)
				 , COALESCE(st.sort_no, s.sort_no) AS sort_no
				 , p.mode_id
				 , p.unit_id
				 , CASE
		...
		FROM #T AS t
			CROSS APPLY (
				SELECT 
					SUM(vp.value) AS add_value_hide
				FROM dbo.View_added_lite AS vp
		...
		--================================
		if @Debug=1 PRINT '@is_out_gis='+str(@is_out_gis)
		if @is_out_gis = 1
		BEGIN
			DECLARE @is_export_gis_without_paid BIT = 0
			SELECT @is_export_gis_without_paid=is_export_gis_without_paid FROM dbo.Occupation_Types WHERE id = @Tip_Id

			if @Debug=1 PRINT '@is_export_gis_without_paid='+str(@is_export_gis_without_paid)
		...
		FROM #T AS t
			JOIN dbo.Services_build AS sb ON (t.service_id = sb.service_id OR t.short_name = sb.service_name)  -- 31.07.2019 добавил OR T.short_name=sb.service_name
		WHERE sb.build_id = t.build_id
		...
		-- Добавляем строки по много-тарифным расчетам
		--IF @Debug=1
		IF @is_out_gis=0
		...
		SELECT dbo.Fun_GetNumUV(occ, @Fin_Id1, @Sup_Id) AS num_pd
			 , * ,(debt+penalty_serv+penalty_prev-paymaccount_peny) AS total_end_period
		FROM #T
		WHERE sup_id = @Sup_Id
		ORDER BY occ
			   , group_sort_id
