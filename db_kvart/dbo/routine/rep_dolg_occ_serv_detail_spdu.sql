...
-- Description:	
-- =============================================
CREATE         PROCEDURE [dbo].[rep_dolg_occ_serv_detail_spdu]
	@occ		INT
	,@fin_id1	SMALLINT
	,@debug		BIT	= NULL
AS
...
exec [rep_dolg_occ_serv_detail2] 1,700009021,149,null,1
exec [rep_dolg_occ_serv_detail2] 1,910000723,142,null
exec [rep_dolg_occ_serv_detail2] 3,680002998,147,323, 1
exec [rep_dolg_occ_serv_detail2] 3,700103553,138,null, 1
*/
BEGIN
	SET NOCOUNT ON;
	...
	--,PRIMARY KEY (occ, service_id)
		...
		(	occ
			...
			WHERE p.occ = @occ
				...
				AND (p.sup_id=0 OR p.sup_id IN (300))
				AND (p.saldo <> 0
				OR p.value <> 0
				OR p.added <> 0
				OR p.paid <> 0
	...
	SELECT
		...
		,'Тариф' =
				CASE
					WHEN u.short_id = 'м2' THEN SUM(tarif)
					ELSE AVG(tarif)
				END
		,'Кол-во' =
			CASE
